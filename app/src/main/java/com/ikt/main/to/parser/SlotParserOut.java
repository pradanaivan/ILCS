...
public class SlotParserOut extends DefaultErrorModel implements IParser{
...
//        }
        try {
            ...
            if(dateArr != null){
                VectorModel.getInstance().clearObjectNewSlotDateOut();
                for (int i = 0; i < dateArr.length(); i++) {
                    String item = dateArr.optString(i);
                    VectorModel.getInstance().setObjectNewSlotsDateOut(item);
                }
            }
            JSONArray slotArr = jsonObject.optJSONArray("slot");
            if(dateArr != null){
                VectorModel.getInstance().clearObjectNewSlotSlotOut();
                for (int i = 0; i < slotArr.length(); i++) {
                    JSONArray slotDetailArr = slotArr.optJSONArray(i);
                    String item = slotDetailArr.toString();
                    VectorModel.getInstance().setObjectNewSlotsSlotOut(item);
                }
            }
            JSONArray areaArr = jsonObject.optJSONArray("area");
            if(dateArr != null){
                VectorModel.getInstance().clearObjectNewSlotAreaOut();
                for (int i = 0; i < areaArr.length(); i++) {
                    String item = areaArr.optString(i);
                    VectorModel.getInstance().setObjectNewSlotsAreaOut(item);
                }
            }
        } catch (JSONException e) {
            setError(e.getMessage());

...
public abstract class BaseActivity2 extends AppCompatActivity {
    ...
    private void prepareMenuToolbar(final Menu menu) {
        ...
        if (!isAvailableMyAccBtn()) {
            final MenuItem myAcc = menu.findItem(R.id.action_my_account);
            myAcc.setVisible(false);
        }
        if (!isAvailableLangBtn()) {
            final MenuItem notif = menu.findItem(R.id.action_notif);
            notif.setVisible(false);
        }
        if (!isAvailableNotifBtn()) {
            final MenuItem language = menu.findItem(R.id.action_language);
